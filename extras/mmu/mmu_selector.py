...
class StepperIdlerSelector(BaseSelector, object):
    ...
    def __init__(self, mmu):
        ...
        self.cad_bypass_offset = mmu.config.getfloat('cad_bypass_offset', 0, minval=0.)
        self.cad_last_gate_offset = mmu.config.getfloat('cad_last_gate_offset', 2.0, minval=0.)
        self.cad_selector_tolerance = mmu.config.getfloat('cad_selector_tolerance', 10.0, above=0.)
    ...
    def handle_connect(self):
        ...
        if self.selector_offsets:
            if len(self.selector_offsets) == self.mmu.num_gates:
                ...
                self.mmu.log_error("Incorrect number of gates in %s" % self.VARS_MMU_SELECTOR_OFFSETS)
                self.selector_offsets = self._ensure_list_size(self.selector_offsets, self.mmu.num_gates)
            if not any(x == -1 for x in self.selector_offsets):
                self.mmu.calibration_status |= self.mmu.CALIBRATED_SELECTOR
        else:
            self.mmu.log_always("Warning: Selector offsets not found in mmu_vars.cfg")
            self.selector_offsets = [-1] * self.mmu.num_gates
        self.mmu.save_variables.allVariables[self.VARS_MMU_SELECTOR_OFFSETS] = self.selector_offsets
        ...
        if self.idler_offsets:
            if len(self.idler_offsets) == self.mmu.num_gates:
                self.mmu.log_debug("Loaded saved idler offsets: %s" % self.idler_offsets)
            else:
                self.mmu.log_error("Incorrect number of gates in %s" % self.VARS_MMU_IDLER_OFFSETS)
                self.idler_offsets = self._ensure_list_size(self.idler_offsets, self.mmu.num_gates)
        else:
            self.mmu.log_always("Warning: Idler offsets not found in mmu_vars.cfg")
            self.idler_offsets = [-1] * self.mmu.num_gates
    ...
    def restore_gate(self, gate):
        if gate >= 0 and gate < len(self.selector_offsets):
            self.set_position(self.selector_offsets[gate])
        self.grip_state = self.mmu.FILAMENT_RELEASE_STATE
    ...
    def get_status(self):
        status = super(StepperIdlerSelector, self).get_status()
        status.update({
            'grip': "Gripped" if self.grip_state == self.mmu.FILAMENT_DRIVE_STATE else "Released",
        })
        return status
    ...
    def _get_max_selector_movement(self, gate=-1):
        n = gate if gate >= 0 else self.mmu.num_gates - 1
        max_movement = self.cad_gate0_pos + (n * self.cad_gate_width)
        max_movement += self.cad_last_gate_offset if gate == self.mmu.TOOL_GATE_BYPASS else 0.
        max_movement += self.cad_selector_tolerance
        return max_movement
    ...
    def _move_selector_to_gate(self, gate):
        if gate >= 0 and gate < len(self.selector_offsets):
            offset = self.selector_offsets[gate]
            if offset >= 0:
                self.mmu.log_trace("Moving selector to gate %d at %.1fmm" % (gate, offset))
                pos = self.mmu_toolhead.get_position()
                pos[0] = offset
                self.mmu_toolhead.move(pos, self.selector_move_speed)
                self.mmu_toolhead.wait_moves()
            else:
                raise MmuError("Selector offset for gate %d not calibrated" % gate)

    def _move_idler_to_gate(self, gate):
        if gate >= 0 and gate < len(self.idler_offsets):
            offset = self.idler_offsets[gate]
            if offset >= 0:
                self.mmu.log_trace("Moving idler to gate %d at %.1fmm" % (gate, offset))
                self.idler_stepper.do_move(movepos=offset, speed=self.idler_move_speed,
                                          accel=self.idler_stepper.accel, sync=True)
            else:
    ...
    cmd_MMU_CALIBRATE_SELECTOR_help = "Calibrate selector positions for specified gate"
    def cmd_MMU_CALIBRATE_SELECTOR(self, gcmd):
        self.mmu.log_to_file(gcmd.get_commandline())
        if self.mmu.check_if_disabled(): return
        ...
        try:
            ...
            self.selector_offsets = [round(self.cad_gate0_pos + i * self.cad_gate_width, 1)
                                    for i in range(self.mmu.num_gates)]
            if save:
                self.mmu.save_variable(self.VARS_MMU_SELECTOR_OFFSETS, self.selector_offsets, write=True)
                self.mmu.log_always("Selector offsets calculated and saved: %s" % self.selector_offsets)
            else:
                self.mmu.log_always("Selector offsets calculated: %s" % self.selector_offsets)
    ...
    cmd_MMU_CALIBRATE_IDLER_help = "Calibrate idler positions for specified gate"
    def cmd_MMU_CALIBRATE_IDLER(self, gcmd):
        ...
        try:
            self.mmu.calibrating = True
            # Calculate positions based on CAD values
            self.idler_offsets = [round(self.cad_idler_gate0_pos + i * self.cad_idler_gate_width, 1)
                                 for i in range(self.mmu.num_gates)]
            if save:
                self.mmu.save_variable(self.VARS_MMU_IDLER_OFFSETS, self.idler_offsets, write=True)
                self.mmu.log_always("Idler offsets calculated and saved: %s" % self.idler_offsets)
            else:
                self.mmu.log_always("Idler offsets calculated: %s" % self.idler_offsets)
        except MmuError as ee:
            self.mmu.handle_mmu_error(str(ee))
        finally:
            self.mmu.calibrating = False

    cmd_MMU_SOAKTEST_SELECTOR_help = "Soak test of selector and idler movement"
    def cmd_MMU_SOAKTEST_SELECTOR(self, gcmd):
        self.mmu.log_to_file(gcmd.get_commandline())
        if self.mmu.check_if_disabled(): return
        if self.mmu.check_if_loaded(): return
        ...
        try:
            ...
            self.mmu.handle_mmu_error("Soaktest abandoned: %s" % str(ee))
